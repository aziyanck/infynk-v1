...
export default function Chatbot() {
    ...
    const sendMessage = async () => {
        ...
        try {
            const response = await fetch('https://aziyan-my-n8n-bot.hf.space/webhook/pixy', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            setMessages(prev => [...prev, { text: "Error connecting to bot.", sender: 'bot', isError: true }]);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <>
            ...
            {/* Chat Window */}
            <div
                className={`fixed bottom-24 right-5 w-80 sm:w-96 h-[500px] bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl z-50 flex flex-col overflow-hidden font-sans transition-all duration-300 origin-bottom-right ${isOpen ? 'scale-100 opacity-100 pointer-events-auto' : 'scale-90 opacity-0 pointer-events-none translate-y-10'}`}
            >
                {/* Header */}
                ...
                <div className="p-3 border-t border-zinc-800 bg-zinc-900 flex gap-2 items-center">
                    ...
                    >
                        <FontAwesomeIcon icon={faPaperPlane} className="text-sm translate-x-[-1px] translate-y-[1px]" />
                    </button>
                </div>
            </div>
